...
- **Global scope**: `C:/Projects/zeref/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Projects/zeref/.cursor/notes/`)
...
2. Glob `C:/Projects/zeref/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (C:/Projects/zeref/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Projects/zeref/.cursor/notes/` — works in any directory
