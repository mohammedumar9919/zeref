  ...
  for (const sub of ["app", "components", "lib"]) {
    for (const abs of collectWebSources(join(webRoot, sub))) {
      ...
      if (C30_INSTAGRAM_IMPORT.test(source)) {
        fail(`C50: ${rel} must not import @zeref/instagram`);
      }

      if (C30_WHISPER_IMPORT.test(source) && !isServerVoicePath(rel)) {
        fail(`C50: ${rel} must not import whisper modules outside app/api/** or lib/voice/**`);
      }

      if (C30_JARVIS_IMPORT.test(source)) {
        if (!isServerVoicePath(rel)) {
          fail(`C50: ${rel} must not import @zeref/jarvis-kernel outside app/api/**, lib/voice/**, or lib/jarvis/**`);
        }
        if (isClientComponent(source)) {
          fail(`C50: ${rel} is a client component and must not import @zeref/jarvis-kernel`);
        }
      }
    }
  }
